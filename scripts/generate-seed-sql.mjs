...
sql.push('begin;');
sql.push('');
sql.push('-- 1) 기존 songs 데이터 + cascade로 user_songs/collection_songs/rooms/song_change_requests 비우기');
sql.push('truncate table songs cascade;');
sql.push('');
sql.push('-- 2) FK 컬럼들 text → uuid 타입 변경 (truncate 후라 cast 필요 없음)');
...
sql.push('');
sql.push('-- 3) songs.id text → uuid + default gen_random_uuid()');
...
sql.push('create or replace function approve_song_change_request(req_id uuid) returns uuid');
